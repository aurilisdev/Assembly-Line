...
public class ScreenBlockBreaker extends GenericOutlineAreaScreen<ContainerBlockBreaker> {
	...
	public ScreenBlockBreaker(ContainerBlockBreaker screenContainer, PlayerInventory inv, ITextComponent titleIn) {
	...
	private List<? extends IReorderingProcessor> getElectricInformation() {
		...
		if (harvester != null) {
			...
			list.add(AssemblyTextUtils.gui("machine.usage", ChatFormatter.getChatDisplayShort(AssemblyLineConstants.BLOCKBREAKER_USAGE * 20, DisplayUnits.WATT)).withStyle(TextFormatting.GRAY).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
	...
	protected List<? extends IReorderingProcessor> getTooltip() {
		...
		if (breaker != null) {
			tips.add(AssemblyTextUtils.tooltip("breakingprogress", ChatFormatter.getChatDisplayShort(100.0 * (double) breaker.ticksSinceCheck.getValue() / (double) breaker.currentWaitTime.getValue(), DisplayUnits.PERCENTAGE)).withStyle(TextFormatting.GRAY).getVisualOrderText());
		}
