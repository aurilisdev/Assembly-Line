...
public class ScreenBlockPlacer extends GenericOutlineAreaScreen<ContainerBlockPlacer> {
	...
	protected List<? extends FormattedCharSequence> getTooltip() {
		...
		if (placer != null) {
			tips.add(AssemblyTextUtils.tooltip("cooldown", placer.currentWaitTime.getValue() - placer.ticksSinceCheck.getValue()).withStyle(ChatFormatting.GRAY).getVisualOrderText());
		}
