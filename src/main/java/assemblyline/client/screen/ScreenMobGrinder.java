...
public class ScreenMobGrinder extends GenericOutlineAreaScreen<ContainerMobGrinder> {
    ...
    public ScreenMobGrinder(ContainerMobGrinder container, PlayerInventory inv, ITextComponent title) {
    ...
    private List<? extends IReorderingProcessor> getElectricInformation() {
        ...
        if (harvester != null) {
            ...
            list.add(AssemblyTextUtils.gui("machine.usage", ChatFormatter.getChatDisplayShort(AssemblyLineConstants.MOBGRINDER_USAGE * harvester.powerUsageMultiplier.getValue() * 20, DisplayUnits.WATT)).withStyle(TextFormatting.GRAY).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
