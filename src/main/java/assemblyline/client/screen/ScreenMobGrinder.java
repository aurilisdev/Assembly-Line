...
public class ScreenMobGrinder extends GenericOutlineAreaScreen<ContainerMobGrinder> {
	...
	addComponent(new ScreenComponentCountdown(this::getTooltip, () -> menu.getSafeHost()
		.map(grinder -> 1.0 - (double) grinder.ticksSinceCheck.getValue() / grinder.currentWaitTime.getValue())
		.orElse(0.0), 10, 50));
	...
	addComponent(new ScreenComponentButton<>(10, 20, 60, 20)
		.setLabel(() -> menu.getSafeHost()
			.map(grinder -> HandlerHarvesterLines.containsLines(grinder.getBlockPos())
	...
	menu.getSafeHost().ifPresent(grinder -> {
	    ...
	    list.add(
		    AssemblyTextUtils
			    .gui("machine.usage",
				    ChatFormatter
					    .getChatDisplayShort(
						    AssemblyLineConfig.getInstance().MOBGRINDER_USAGE.getAsDouble()
							    * grinder.powerUsageMultiplier.getValue() * 20,
						    DisplayUnits.WATT))
			    .withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.DARK_GRAY).getVisualOrderText());

	    ComponentElectrodynamic electro = grinder.requireComponent(IComponentType.Electrodynamic);
	...
	menu.getSafeHost()
		.ifPresent(grinder -> tips.add(AssemblyTextUtils
			.tooltip("cooldown", grinder.currentWaitTime.getValue() - grinder.ticksSinceCheck.getValue())
