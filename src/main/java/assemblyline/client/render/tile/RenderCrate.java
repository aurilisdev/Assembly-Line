...
public class RenderCrate extends AbstractTileRenderer<TileCrate> {
    ...
    @Override
    public void render(TileCrate tileCrate, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn,
	...
	for (Direction dir : Direction.values()) {
		...
		matrixStackIn.translate(0.5 + dir.getStepX() / 1.999, 0.3 + dir.getStepY() / 2.0,
			0.5 + dir.getStepZ() / 1.999);
		switch (dir) {
		case EAST:
		...
		for (ItemStack stack : tileCrate.<ComponentInventory>getComponent(IComponentType.Inventory)
			.getItems()) {
		    if (!stack.isEmpty()) {
			item = stack;
			break;
		...
		Component displayNameIn = Component
			.literal(tileCrate.getCount() + "x" + item.getHoverName().getString());
		...
		fontrenderer.drawInBatch(displayNameIn, f2, 0, 0, false, matrix4f, bufferIn, DisplayMode.NORMAL, 0,
			combinedLightIn);
		...
		Minecraft.getInstance().getItemRenderer().renderStatic(stack, ItemDisplayContext.NONE, 0xF000F0,
			combinedOverlayIn, matrixStackIn, bufferIn, tileCrate.getLevel(), 0);
