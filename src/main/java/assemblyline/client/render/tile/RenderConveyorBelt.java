...
public class RenderConveyorBelt extends AbstractTileRenderer<TileConveyorBelt> {
	...
	case VERTICAL -> {
	    if (level.getBlockEntity(tile.getBlockPos().below()) instanceof GenericTileConveyorBelt belt
		    && belt.getConveyorType() == ConveyorType.VERTICAL) {
		if (!(level.getBlockEntity(tile.getBlockPos().above()) instanceof GenericTileConveyorBelt)) {
	    ...
	    } else if (!(level.getBlockEntity(tile.getBlockPos().above()) instanceof GenericTileConveyorBelt)) {
	...
	RenderingUtils.renderModel(getModel(location), RenderType.solid(), matrixStackIn, bufferIn, combinedLightIn,
		combinedOverlayIn);
	matrixStackIn.popPose();
	...
	if (tile.isPusher.getValue()) {
	    BlockPos nextBlockPos = tile.getNextPos(level).subtract(tile.getBlockPos());
	    matrixStackIn.pushPose();
	    matrixStackIn.translate(0, 1 / 16.0, 0);
	    if (type == ConveyorType.SLOPED_DOWN)
	    ...
	    RenderingUtils.prepareRotationalTileModel(tile, matrixStackIn);
	    RenderingUtils.renderModel(model, RenderType.solid(), matrixStackIn, bufferIn, combinedLightIn,
		    combinedOverlayIn);
	    matrixStackIn.popPose();
	}
