...
public class RenderConveyorBelt extends AbstractTileRenderer<TileConveyorBelt> {
    ...
    @Override
    public void render(TileConveyorBelt tile, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn,
	...
	if (!stack.isEmpty()) {
		...
		matrixStackIn.translate(itemVec.x(), itemVec.y() + (blockItem ? 0.167 : 0.05f / 16.0f) + move.y(),
			itemVec.z());
		...
		// matrixStackIn.mulPose(direct == Direction.NORTH ?
		// Vector3f.XN.rotationDegrees(rotate) : direct == Direction.SOUTH ?
		// Vector3f.XP.rotationDegrees(-rotate) : direct == Direction.WEST ?
		// Vector3f.XN.rotationDegrees(rotate) :
	    ...
	    minecraft().getItemRenderer().renderStatic(stack, ItemDisplayContext.NONE, combinedLightIn,
		    combinedOverlayIn, matrixStackIn, bufferIn, tile.getLevel(), 0);
	}
	...
	case SLOPED_DOWN -> tile.running.getValue() ? AssemblyLineClientRegister.MODEL_SLOPEDCONVEYORDOWNANIMATED
		: AssemblyLineClientRegister.MODEL_SLOPEDCONVEYORDOWN;
	case SLOPED_UP -> tile.running.getValue() ? AssemblyLineClientRegister.MODEL_SLOPEDCONVEYORUPANIMATED
		: AssemblyLineClientRegister.MODEL_SLOPEDCONVEYORUP;
	case VERTICAL -> {

	    if (tile.getLevel().getBlockEntity(tile.getBlockPos().below()) instanceof GenericTileConveyorBelt belt
		    && belt.getConveyorType() == ConveyorType.VERTICAL) {
		if (!(tile.getLevel().getBlockEntity(tile.getBlockPos().above()) instanceof GenericTileConveyorBelt)) {
		    yield tile.running.getValue() ? AssemblyLineClientRegister.MODEL_ELEVATORRUNNINGBUTLAST
			    : AssemblyLineClientRegister.MODEL_ELEVATORBUTLAST;

		}
		yield tile.running.getValue() ? AssemblyLineClientRegister.MODEL_ELEVATORRUNNING
			: AssemblyLineClientRegister.MODEL_ELEVATOR;

	    } else if (!(tile.getLevel()
		    .getBlockEntity(tile.getBlockPos().above()) instanceof GenericTileConveyorBelt)) {
		yield tile.running.getValue() ? AssemblyLineClientRegister.MODEL_ELEVATORBOTTOMRUNNINGBUTLAST
			: AssemblyLineClientRegister.MODEL_ELEVATORBOTTOMBUTLAST;

	    }
	    yield tile.running.getValue() ? AssemblyLineClientRegister.MODEL_ELEVATORBOTTOMRUNNING
		    : AssemblyLineClientRegister.MODEL_ELEVATORBOTTOM;

	}
	default -> tile.running.getValue() ? AssemblyLineClientRegister.MODEL_CONVEYORANIMATED
		: AssemblyLineClientRegister.MODEL_CONVEYOR;
	};

	RenderingUtils.renderModel(getModel(location), tile, RenderType.solid(), matrixStackIn, bufferIn,
	...
	if (tile.isPusher.getValue()) {
	    ...
	    matrixStackIn.translate(nextBlockPos.getX() - move.x(), nextBlockPos.getY() - move.y(),
		    nextBlockPos.getZ() - move.z());

	    RenderingUtils.prepareRotationalTileModel(tile, matrixStackIn);

	    RenderingUtils.renderModel(model, tile, RenderType.solid(), matrixStackIn, bufferIn, combinedLightIn,
		    combinedOverlayIn);
