...
public class RenderBlockBreaker extends AbstractTileRenderer<TileBlockBreaker> {
	...
	BakedModel ibakedmodel = Minecraft.getInstance().getModelManager()
		.getModel(AssemblyLineClientRegister.MODEL_BLOCKBREAKERWHEEL);
	...
	VertexConsumer vertexconsumer1 = new SheetedDecalTextureGenerator(
		Minecraft.getInstance().renderBuffers().crumblingBufferSource()
			.getBuffer(ModelBakery.DESTROY_TYPES.get(Math.min(9, (int) (breaker.progress.getValue() * 9)))),
