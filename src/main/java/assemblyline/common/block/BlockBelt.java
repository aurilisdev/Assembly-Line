...
public class BlockBelt extends Block {
	...
	return Arrays.asList(new ItemStack(DeferredRegisters.blockBelt));
    }
	...
	if (!world.isRemote && tile instanceof TileBelt && entityIn instanceof ItemEntity) {
	    TileBelt belt = (TileBelt) tile;
	...
	if (!(newState.getBlock() instanceof BlockBelt)) {
	    TileEntity tile = worldIn.getTileEntity(pos);
	    if (!(state.getBlock() == newState.getBlock() && state.get(BlockGenericMachine.FACING) != state.get(BlockGenericMachine.FACING))
