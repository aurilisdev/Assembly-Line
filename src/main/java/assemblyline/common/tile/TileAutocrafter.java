...
public class TileAutocrafter extends GenericTile {
	...
	addComponent(
		new ComponentElectrodynamic(this, false, true).maxJoules(AssemblyLineConstants.AUTOCRAFTER_USAGE * 20)
	...
	for (int i = 0; i <= 3 - shaped.getWidth(); ++i) {
		...
		if (TileAutocrafter.shapedMatches(inv, shaped, i, j, true)
			|| TileAutocrafter.shapedMatches(inv, shaped, i, j, false)) {
		    return true;
		}
	    }
	}
    ...
    private static boolean shapedMatches(ComponentInventory inventory, ShapedRecipe shaped, int x, int y,
	    boolean flag) {
	for (int i = 0; i < 3; ++i) {
	    for (int j = 0; j < 3; ++j) {
		int k = i - x;
	...
	if (tick.getTicks() % 20 == 0) {
		...
		if (canContinue) {
		    List<CraftingRecipe> recipes = level.getServer().getRecipeManager()
			...
			if (currentItemStack.isEmpty() || ItemStack.isSameItem(result, currentItemStack)
