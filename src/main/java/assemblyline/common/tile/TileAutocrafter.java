	...
	public void tickServer(ComponentTickable tick) {
		...
		if (tick.getTicks() % 20 == 0) {
			if (canContinue) {
				...
				if (canContinue) {
					List<ICraftingRecipe> recipes = level.getServer().getRecipeManager().getAllRecipesFor(IRecipeType.CRAFTING);
					ItemStack result = ItemStack.EMPTY;
					canContinue = false;
					for (ICraftingRecipe recipe : recipes) {
						result = recipe.getResultItem();
						if (recipe instanceof ShapedRecipe) {
							if (shapedMatches(inventory, (ShapedRecipe) recipe)) {
								canContinue = true;
								break;
							}
						} else if (recipe instanceof ShapelessRecipe) {
							if (shapelessMatches(inventory, (ShapelessRecipe) recipe)) {
								canContinue = true;
								break;
							}
						}
