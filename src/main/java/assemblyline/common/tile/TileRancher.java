...
public class TileRancher extends TileOutlineArea {
	...
	addComponent(new ComponentElectrodynamic(this, false, true)
		...
		.maxJoules(AssemblyLineConfig.getInstance().RANCHER_USAGE.getAsDouble() * 20));
	addComponent(new ComponentInventory(this, ComponentInventory.InventoryBuilder.newInv().outputs(9).upgrades(3))
		...
		//
		.setDirectionsBySlot(1, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		//
		.setDirectionsBySlot(2, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		//
		.setDirectionsBySlot(3, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		//
		.setDirectionsBySlot(4, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		//
		.setDirectionsBySlot(5, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		//
		.setDirectionsBySlot(6, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		//
		.setDirectionsBySlot(7, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		//
		.setDirectionsBySlot(8, BlockEntityUtils.MachineDirection.TOP, BlockEntityUtils.MachineDirection.BOTTOM,
			BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.RIGHT)
		.validUpgrades(ContainerRancher.VALID_UPGRADES).valid(machineValidator()));
	addComponent(new ComponentContainerProvider("rancher", this).createMenu((id, player) -> new ContainerRancher(id,
	...
	if (electro.getJoulesStored() < AssemblyLineConfig.getInstance().RANCHER_USAGE.getAsDouble()
		|| !inv.areOutputsEmpty()) {
	    return;
	}
	...
	for (Entity entity : entities) {
	    ...
	    if (entity instanceof IShearable sheep
		    && sheep.isShearable(null, new ItemStack(Items.SHEARS), level, entity.blockPosition())) {

		collectedItems
			.addAll(sheep.onSheared(null, new ItemStack(Items.SHEARS), level, entity.blockPosition()));

		electro.joules(
			electro.getJoulesStored() - AssemblyLineConfig.getInstance().RANCHER_USAGE.getAsDouble());
