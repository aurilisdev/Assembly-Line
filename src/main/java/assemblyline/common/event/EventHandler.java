...
public class EventHandler {
	...
	public static void attachLocStorageCapabiliity(AttachCapabilitiesEvent<Entity> event) {
		...
		if (entity.getCapability(VoltaicCapabilities.CAPABILITY_LOCATIONSTORAGE_ITEM).orElse(AssemblyCapabilityUtils.EMPTY_LOCATION) == AssemblyCapabilityUtils.EMPTY_LOCATION) {
	...
	public static void captureDroppedItems(LivingDropsEvent event) {
		...
		ILocationStorage storage = entity.getCapability(VoltaicCapabilities.CAPABILITY_LOCATIONSTORAGE_ITEM).orElse(AssemblyCapabilityUtils.EMPTY_LOCATION);
		if (storage == AssemblyCapabilityUtils.EMPTY_LOCATION) {
			return;
		}
		World level = entity.level;
		Location location = storage.getLocation(0);
		TileEntity machine = level.getBlockEntity(new BlockPos(location.intX(), location.intY(), location.intZ()));
		if (machine instanceof TileMobGrinder) {
			TileMobGrinder grinder = (TileMobGrinder) machine;
