...
public class AssemblyLineLangKeyProvider extends BaseLangKeyProvider {
	...
	default:
	    ...
	    addBlock(AssemblyLineBlocks.BLOCK_CONVEYORBELT, "Conveyor Belt");
	    addBlock(AssemblyLineBlocks.BLOCK_SORTERBELT, "Sorter Belt");
	    addBlock(AssemblyLineBlocks.BLOCK_DETECTOR, "Detector");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.crate), "Crate");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.cratemedium),
		    "Medium Crate");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.cratelarge),
		    "Large Crate");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.autocrafter),
		    "Autocrafter");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.blockbreaker),
		    "Block Breaker");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.blockplacer),
		    "Block Placer");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.rancher),
		    "Energized Rancher");
	    addBlock(AssemblyLineBlocks.BLOCKS_ASSEMBLYMACHINES.getValue(SubtypeAssemblyMachine.mobgrinder),
		    "Mob Grinder");
	    ...
	    // addItem(AssemblyLineItems.ITEM_SPEEDUPGRADE_ADVANCED, "Advanced Speed
	    // Upgrade");
	    ...
	    addGuidebook("chapter.conveyers.l1",
		    "Conveyor Belts provide a robust method for moving items between inventories. Requiring only 8 J/t or 160W at 120 V to run, they offer a valuable alternative to moving items manually. Furthermore, only one belt in a segment needs to have a cable attached, as it will distribute power to the other belts. Power is fed in from "
			    + "the bottom of the conveyor.");
	    addGuidebook("chapter.conveyers.l2",
		    "The yellow arrows on the belt represents the direction items will be transferred. To load items onto a belt, you can either drop the item and the belt will pick it up, or the belt can extract items from an inventory. If a belt does not end in an inventory, then it will drop its items onto the ground. Conveyors will also push mobs"
			    + " and players that walk over them if the belt is running. You can wear Rubber Boots to counteract this. Conveyor Belts have 4 modes they can switch between by Right-Clicking a wrench on them. These are:");
	    ...
	    addGuidebook("chapter.conveyers.l3",
		    "These modes are pictured on the following pages. Note where the bounding boxes are:");
	    ...
	    addGuidebook("chapter.conveyers.l5",
		    "Another useful block is the Detector. The Detector will output a 15 redstone signal if an item on a Conveyor Belt passes in front of the green square of the Detector, or if an item is laying on the ground in front of said square.");

	    addGuidebook("chapter.machines", "Machines");
	    addGuidebook("chapter.machines.blockbreaker",
		    "The Block Breaker will break any block placed in front of it as long as it is minable with a pickaxe. Use the \"Show Area\" button to view the block it is mining. The progress bar counts down the break time left on the block that is being mined.");

	    addGuidebook("chapter.machines.blockplacer",
		    "The Block Placer will place any block added to its inventory in front of it. Use the \"Show Area\" button to view the area it can place a block in. The progress bar counts down the time between block placements.");
	    ...
	    addGuidebook("chapter.machines.mobgrinder",
		    "The Mob Grinder will kill any mobs in its working area. Use the \"Show Area\" button to view the area it is currently covering. The progress bar counts down the time between checks. The Grinder can use up to 12 Range Upgrades to expand its working area.");

	    addGuidebook("chapter.machines.farmer1",
		    "The Farmer will plant and harvest crops and trees within its working area. The colors of the input slots corrospond to the regions the items will be planted in. The Farmer must be placed underneath the land it is working. The top right slot is for Bone Meal.");
